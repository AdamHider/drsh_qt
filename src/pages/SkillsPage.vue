<template>
  <q-page-container>
    <q-app-header class="transparent text-white rounded-b-md" reveal :showForce="headerShowForce">
      <UserResourceBar v-if="user.active?.data.resources" :resource="user.active?.data.resources.cobalt" dense no-caption size="24px" push />
      ...
      <UserResourceBar v-if="user.active?.data.resources" :resource="user.active?.data.resources.rubidium" dense no-caption size="24px" push />
    </q-app-header>
