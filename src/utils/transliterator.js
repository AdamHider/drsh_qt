...
function translit(input, exceptions, mode = 'simple'){
    ...
    //input = ' '+input+' ';
    if(mode == 'inlude_tags'){
        for ( var k in html_exceptions ) {
            input = input.replace( html_exceptions[k][0], '['+k+']' );
        }
    }
...
function makeTagSubstitution(input){
    for(var i = 0; i < html_exceptions.length; i++){
        input = input.replace(new RegExp('\\['+i+'\\]',"g"), html_exceptions[i][1]);
    }
    return input;
}
